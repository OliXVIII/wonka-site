...
export const createImageForContent = async (mission: string, subject: string, image: string): Promise<string> => {
  const prompt = await createImagePrompt(mission, subject, image);
